-- Update the RPC function to also set devices.child_id for consistency
-- This ensures both approaches (direct devices.child_id and device_child_assignments) work
...
begin
  ...
  -- If setting as active, deactivate all other assignments for this device first
  if _is_active then
  ...
  -- Also update devices.child_id for consistency when setting active
  if _is_active then
