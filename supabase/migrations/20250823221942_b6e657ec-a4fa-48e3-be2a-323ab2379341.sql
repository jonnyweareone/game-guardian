...
create policy if not exists p_devices_select_owner
  on public.devices for select
  using (owner_user = auth.uid());

create policy if not exists p_configs_select_owner
...
create policy if not exists p_heartbeats_select_owner
