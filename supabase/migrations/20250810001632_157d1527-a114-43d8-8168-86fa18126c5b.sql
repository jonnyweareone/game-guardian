...
CREATE TABLE public.location_match_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
...
CREATE POLICY "Users can view their own location checks"
ON public.location_match_results
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own location checks"
ON public.location_match_results
FOR INSERT
WITH CHECK (auth.uid() = user_id);
